...
from Utils.constants import PROCESSED_DIR,RESULTS_DIR,RAW_DIR
...
x=df.drop(columns=['Historical_Cost_of_Ride']) #Feature column
...
transformer= ColumnTransformer(transformers=[
    # Apply OneHotEncoder for Nominal columns
        ...
        ('scaler', StandardScaler(), ['Number_of_Riders', 'Number_of_Drivers','Average_Ratings','Number_of_Past_Rides','Expected_Ride_Duration'])
    ],
...
y_pred_rf=rf.predict(X_test_trns)
...
y_pred_lr=lr.predict(X_test_trns)
...
output_file = os.path.join(RESULTS_DIR, "test_results by model_3.csv")
results_df.to_csv(output_file, index=False)

df_matrics =pd.DataFrame(columns=['Model','Model_Description','Train_Error','Val_Error','Test_Error'])
df_matrics.loc[len(df_matrics)]=['Model_3 (Random Forest)','Using numerics + categorical columns (applying Scaling and Encoding)',train_rmse,val_rmse,test_rmse]
df_matrics.loc[len(df_matrics)]=['Model_3 (Linear Regression)','Using numerics + categorical columns (applying Scaling and Encoding)',train_rmse_lr, val_rmse_lr, test_rmse_lr]
