...
x=df[['Number_of_Riders','Number_of_Drivers','Number_of_Past_Rides','Average_Ratings','Expected_Ride_Duration']] #Feature column
...
output_file = os.path.join(RESULTS_DIR, "test_results by model_1.csv")
results_df.to_csv(output_file, index=False)

df_matrics =pd.DataFrame(columns=['Model','Model_Description','Train_Error','Val_Error','Test_Error'])
df_matrics.loc[len(df_matrics)]=['Model_1 (Random Forest)','Using only numerics columns',train_rmse,val_rmse,test_rmse]
df_matrics.loc[len(df_matrics)]=['Model_1 (Linear Regression)','Using only numerics columns',train_rmse_lr, val_rmse_lr, test_rmse_lr]
